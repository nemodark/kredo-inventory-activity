...
class Purchase extends Database{
    ...
    public function buy($item_id, $purchase_quantity){
        ...
        $result = $this->conn->query($sql);
        if($result){
            $sql = "INSERT INTO purchase(item_id, quantity, subtotal, purchase_status) VALUES('$item_id', '$purchase_quantity', '$total', 'pending')";
            $result = $this->conn->query($sql);
            if($result){
